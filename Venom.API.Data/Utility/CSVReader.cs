...
namespace Venom.API.Data.Utility
{
    ...
    internal class CsvReader
    {
        ...
        public DateTimeOffset? ModifyDate => _responseMessage.Content.Headers.LastModified;
        ...
        public async Task<bool> LoadFileAsync()
        {
            ...
            {
                _responseMessage = await client.GetAsync( _uri );
            }
            return _responseMessage.IsSuccessStatusCode;
        }

        public async Task<IReadOnlyList<T>> DownloadFileAsync<T>( Uri uri, CreateEntry<T> creator )
        {
        ...
        public async Task<IReadOnlyList<T>> ReadFileAsync<T>( CreateEntry<T> creator )
        {
            var items = new ConcurrentBag<T>( );
        ...
        public async Task<IReadOnlyList<T>> ReadFileAsync<T>( string path, CreateEntry<T> creator )
        {
            var items = new ConcurrentBag<T>( );
        ...
        private async Task ReadStream<T>( Stream stream, CreateEntry<T> creator, ConcurrentBag<T> items )
        {
        ...
        private Task CreateBufferReadTask<T>( CsvBuffer buffer, CreateEntry<T> creator, ConcurrentBag<T> items )
        {
